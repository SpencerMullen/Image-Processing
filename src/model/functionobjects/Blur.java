...
/**
 * Class representing a blur transformation. This can be applied to any 2d array of pixels,
 * and will subsequently alter the color channels of each pixel to produce a blurred image.
 */
public class Blur extends AFilter {
  ...
  /**
   * Creates a blur object with an operation that represents the kernel, starting from the top left,
   * then going row by row.
   */
  public Blur() {
    this.operation = new ArrayList<>(9);

    this.operation.add(0.0625);
  ...
  public ArrayList<ArrayList<IPixel>> apply(ArrayList<ArrayList<IPixel>> image) {
    ...
    for (int y = 0; y < image.size(); y++) {
      for (int x = 0; x < image.get(0).size(); x++) {
        tempList = this.getSurrounding(y, x, image, 3);
        pixelRed = 0;
        pixelGreen = 0;
        pixelBlue = 0;
